...
### Personal Data - EDA
```{r}
PD <- readxl::read_excel("PersonData.xlsx")
...
Y <- ST01$Total.ST.min
X1 <- ST01$Social.ST.min
X2 <- ST01$Pickups
X3 <- ST01$Daily.Course.Hour
X4 <- ST01$Daily_Proportion_Social_Screen_Time
X5 <- ST01$Snow.NotSnow
X6 <- ST01$Daily_Duration_Per_Use
X <- c(X1, X2, X3)
...
```{r}
model <- lm(Total.ST.min ~ Pickups + Daily.Course.Hour + Pickups + factor(Snow.NotSnow) , data = ST01)
summary(model)
